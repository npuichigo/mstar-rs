...
| T6 | HTTP frontend — scale path (`crates/mstar-server`, axum) | ▢ planned | At high throughput the Python API layer saturates (why vLLM and SGLang added Rust frontends): the bottleneck is per-request **tokenize** + per-token **detokenize + SSE/JSON serialize** under the GIL/event-loop, *not* the conductor (per-forward-pass, descriptor-only, GPU-bound). Plan: a Rust **axum** frontend doing HTTP + OpenAI routes + HF `tokenizers`-crate tokenize/detokenize + per-token SSE all off the GIL, talking to the Python conductor over the `Mailbox` (submit token-ids, receive token-ids). Result: no Python on the request/token hot path — an all-Rust control+transport+frontend stack, Python only at torch execution. |
