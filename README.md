...
| T5 dependent | `crates/mstar-comm` | ◐ transport done | Multi-process transport, both planes landed. **Control plane**: a PUSH/PULL message `Mailbox` (bind one inbox, fire-and-forget sends by id, ordered delivery, reconnect-on-restart via socket-inode check) over Unix-domain sockets + length-prefixed bincode frames — a real wire format replacing pickle, no libzmq dependency. **Data plane**: `ShmArena` — a persistent `/dev/shm` mmap + first-fit free-list allocator (ported self-contained), exposed to Python via the buffer protocol (`mstar-py`), so `torch.frombuffer(memoryview(arena)[off:off+n])` moves tensors cross-process zero-copy (verified producer→consumer across two processes). Next: splitting the in-process runtime into conductor + worker processes over these primitives. |
