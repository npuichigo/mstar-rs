...
| **T5** | Multi-process transport (`crates/mstar-comm`) | ✅ implemented | Both planes. **Control**: a PUSH/PULL message `Mailbox` over **ZeroMQ** (`zmq` crate / libzmq) — each entity binds one PULL inbox at `ipc://<dir>/<id>.ipc` and connects a PUSH socket per peer; fire-and-forget, ordered, with libzmq handling queue-to-unbound-peer and auto-reconnect-on-restart. Payloads are bincode-serialized (a real wire format replacing pickle), one message per zmq frame. This mirrors mstar's own ZMQ PUSH/PULL layer. **Data**: `ShmArena` — a persistent `/dev/shm` mmap + first-fit free-list allocator, exposed to Python via the buffer protocol so `torch.frombuffer(memoryview(arena)[off:off+n])` moves tensors cross-process zero-copy. |
| **T5** | Runtime split (`python/mstar_rs/dist.py`) | ✅ implemented | **Self-driving workers + per-request coordinator (decentralized).** Each worker owns its partition(s) and a local `Driver` (scheduler + Rust `Runtime` + engine co-located) — the per-token decode loop pays no central round-trip (mstar's in-worker scheduler model). Cross-partition tensors stream worker→worker peer-to-peer over `/dev/shm` with only descriptors on the wire; a thin coordinator seeds each request once and relays emissions. Proven by `examples/verify_disagg.py` and the TP/streaming/crash verifiers alongside it. |
| **T5** | Serving drive-loop (`python/mstar_rs/server.py`) | ✅ implemented | `ServingEngine`: a background drive thread owns the `Conductor` **or** an in-process `Driver` (all Rust-`Runtime` access on one thread, as PyO3 requires) and loops `poll()` — continuous-batching across every in-flight request; callers enqueue a request and await completion. HTTP-agnostic — the HTTP surface is T6. (An earlier in-process FastAPI dev app was removed once T6 covered the whole API surface.) |
| **T6** | HTTP frontend (`crates/mstar-server`, axum) + launcher (`python/mstar_rs/launch.py`) | ✅ implemented | The **only** HTTP server — a faithful port of mstar's `api_server` surface to Rust: `/v1/models`, `/v1/chat/completions` (SSE + JSON; text/audio/image out), `/v1/audio/speech` (streaming WAV or container), `/v1/images/generations` + `/edits`, native `/generate` (multipart → NDJSON), `/health`. Model-agnostic endpoints + per-model **adapters** (`adapters.rs`), OpenAI **protocol** models with `extra_body` passthrough (`protocol.rs`), inbound/outbound **media** handling (data-URL/base64 → upload files; PCM16 → WAV/streaming-WAV; PNG → data-URL — `media.rs`). HTTP, JSON, base64 and SSE all run off the GIL, which is exactly why **vLLM (`rust/vllm-server`) and SGLang (`sgl-router`) moved their front-ends to Rust**. The **bridge** (`bridge.rs`) speaks msgpack over the same zmq mesh to the Python conductor: submit `{text, file_paths, in/out modalities, model_kwargs}`, stream `ResultChunk {modality, data, metadata}` back — tokenization/feature-extraction stay with the model (conductor policy + engine), matching mstar's data-plane contract. `python -m mstar_rs.launch` starts workers + conductor bridge + the axum binary as one command (`--speech` adds the Talker/Code2Wav partition; `--no-frontend` runs the backend alone so `mstar-server` can be deployed as a separate service). Proven live end-to-end: text/image/audio-in → text-out and speech-out (`/v1/audio/speech` TTS; chat with `modalities:["text","audio"]` returning `message.audio` WAV) with zero Python in the HTTP path. |
