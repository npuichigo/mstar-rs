...
| T5 dependent | `crates/mstar-comm` | ◐ control mesh done | Multi-process transport. **Control mesh landed**: a PUSH/PULL message `Mailbox` (each entity binds one inbox, fire-and-forget sends to peers by id, ordered delivery, transparent reconnect on peer restart via socket-inode check) over **Unix-domain sockets + length-prefixed bincode frames** — a real wire format replacing mstar's pickle, no libzmq dependency. Next: SHM-ring tensor transport (grow `mstar/rust/mstar_shmring`) + splitting the in-process runtime into conductor + worker processes. |
