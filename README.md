...
| **T6** | HTTP frontend — scale path (`crates/mstar-server`, axum) | ◐ frontend proven | Why: at high throughput the Python API layer saturates (vLLM and SGLang both added Rust frontends) — the bottleneck is per-request **tokenize** + per-token **detokenize + SSE/JSON serialize** under the GIL, *not* the conductor (per-forward-pass, descriptor-only, GPU-bound). **Landed**: a Rust axum server doing HTTP + `/tokenize` / `/detokenize` + a streaming `/v1/chat/completions` with **HF `tokenizers`-crate** tokenization and **incremental detokenization + per-token SSE**, all off the GIL (proven end-to-end against a real gpt2 tokenizer). Next: replace the mock token source with a `Mailbox` bridge to the Python conductor (submit token-ids, receive a token-id stream), so no Python touches the request/token hot path — an all-Rust control+transport+frontend stack, Python only at torch execution. |
