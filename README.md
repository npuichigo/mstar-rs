...
| **T5** | HTTP server — dev path (`python/mstar_rs/server.py`) | ✅ implemented | A **FastAPI** surface co-located with the conductor. A background drive thread owns the `Conductor` (all Rust-`Runtime` access on one thread, as PyO3 requires) and loops `conductor.poll()` — continuous-batching across every in-flight request and dispatching to workers; HTTP handlers enqueue a request and await completion. Proven end-to-end by `examples/serve.py`: `POST /generate` → serving engine → zmq dispatch → worker execute → SHM tensors → result over HTTP. Right for dev / low-concurrency. |
| **T6** | HTTP frontend — scale path (`crates/mstar-server`, axum) | ✅ implemented | Why: under **high concurrency** a Python FastAPI/uvicorn API layer saturates on the GIL — per-request **tokenize** + per-token **detokenize + SSE/JSON serialize** all contend for one interpreter lock — which is exactly why **both vLLM and SGLang moved their front-ends to Rust**. The bottleneck is the front-end, *not* the conductor (per-forward-pass, descriptor-only, GPU-bound). A Rust axum server does HTTP + `/tokenize` / `/detokenize` + a streaming `/v1/chat/completions` with **HF `tokenizers`-crate** tokenization and **incremental detokenization + per-token SSE**, all off the GIL. The **`Mailbox` bridge** (`crates/mstar-server/src/bridge.rs`) connects it to the Python conductor: the frontend submits token-ids and receives a token-id stream back (msgpack over the same zmq mesh), so **no Python touches the request/token hot path** — an all-Rust control+transport+frontend stack, Python only at torch execution. Proven end-to-end by `examples/serve_bridge.py`: `curl` a streaming completion → axum tokenizes → conductor policy → worker `execute` → per-token emissions stream back → Rust detokenizes + SSE → the reassembled text matches the prompt exactly. |
